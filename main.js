const btnDocs3 = document.getElementById("btn-3");
const btnDocs4 = document.getElementById("btn-4");
const btnDocs5 = document.getElementById("btn-5");
const btnDocs6 = document.getElementById("btn-6");
const btnDocs7 = document.getElementById("btn-7");
const btnDocs9 = document.getElementById("btn-9");
const btnDocs11 = document.getElementById("btn-11");
const btnDocs12 = document.getElementById("btn-12");
const btnDocs13 = document.getElementById("btn-13");
const btnDocs14 = document.getElementById("btn-14");
const btnDocs15 = document.getElementById("btn-15");
const btnDocs16 = document.getElementById("btn-16");
const btnDocs17 = document.getElementById("btn-17");

btnDocs3.addEventListener("click", () => {
  window.open("https://github.com/AliceRamirez17/LIM012-data-lovers", "_blank");
});

btnDocs4.addEventListener("click", () => {
  window.open(
    "https://aliceramirez17.github.io/LIM012-data-lovers/src/",
    "_blank"
  );
});

btnDocs5.addEventListener("click", () => {
  window.open(
    "https://github.com/AliceRamirez17/LIM012-fe-social-network/tree/gh-pages",
    "_blank"
  );
});

btnDocs6.addEventListener("click", () => {
  window.open(
    "https://aliceramirez17.github.io/LIM012-fe-social-network/src/#/signin",
    "_blank"
  );
});

btnDocs7.addEventListener("click", () => {
  window.open("https://aliceramirez17.github.io/LIM012-fe-md-links/", "_blank");
});

btnDocs11.addEventListener("click", () => {
  window.open("https://www.behance.net/gallery/201383967/Landing-Page-UX-UI-Organic-Beauty", "_blank");
});

btnDocs12.addEventListener("click", () => {
  window.open("https://www.behance.net/gallery/201460835/Landing-Page-Tiendas-Mass-UX-UI", "_blank");
});

btnDocs13.addEventListener("click", () => {
  window.open("https://www.tiendasmass.com.pe/", "_blank");
});

btnDocs14.addEventListener("click", () => {
  window.open("https://www.behance.net/gallery/201535843/Landing-Page-Lexington-Apartments-UX-UI", "_blank");
});

btnDocs15.addEventListener("click", () => {
  window.open("https://lexingtongarden.com/", "_blank");
});

btnDocs16.addEventListener("click", () => {
  window.open("https://kiviu.com/", "_blank");
});

btnDocs17.addEventListener("click", () => {
  window.open("https://cuandojugamos.com/", "_blank");
});

const btnLinkedin = document.getElementById("btnLinkedin");
const btnGithub = document.getElementById("btnGithub");
const btnBehance = document.getElementById("btnBehance");

btnLinkedin.addEventListener("click", () => {
  window.open(
    "https://www.linkedin.com/in/alice-ramirez-developer/",
    "_blank"
  );
});

btnGithub.addEventListener("click", () => {
  window.open("https://github.com/AliceRamirez17", "_blank");
});

btnBehance.addEventListener("click", () => {
  window.open("https://www.behance.net/aliceramirez", "_blank");
});
